...
def processLine(l):	
	...
	try:
		...
		if "authentication successful" in l:
			message = l
			user_id =  l.split("username=")[1].split(" ")[0].strip().strip("%").strip("%@umass.edu")

			mac_id = l.split("mac=")[1].split(" ")[0].strip().strip(":").strip("#")
			...
			if len(a)!=1:
				uMac = a[1].strip().split(" ")[0].strip().strip(":").strip("#")
			else:
				b = l.split("deauth to sta:")
				uMac = b[1].strip().split(" ")[0].strip().strip(":").strip("#")
